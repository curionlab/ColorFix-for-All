...
export function buildRecommendationReason(before, after, scores) {
    ...
    if (Math.abs(lDiff) > 5) {
        if (lDiff > 0)
            reasons.push('明るさを上げて背景との差を広げました');
        else
            reasons.push('明るさを少し下げて背景との差を広げました');
    }
    const sDiff = hslAfter.s - hslBefore.s;
    if (Math.abs(sDiff) > 10) {
        if (sDiff > 0)
            reasons.push('彩度を上げて色を鮮やかにしました');
        else
            reasons.push('彩度を抑えて近い色との混同を減らしました');
    }
    if (reasons.length === 0) {
